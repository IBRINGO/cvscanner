"""The reusable core of the Phase 4 matching pipeline (sections 5-24),
extracted from RunAnalysis so Phase 5's tailoring re-analysis step
(section 73: "Do not manually modify the score... the score must come
from the existing Phase 4 engine") calls the exact same code path as a
fresh analysis - never a second, parallel scoring implementation that
...
from domain.matching.entities import Gap, RequirementEvaluation, ScoreBreakdown
...
class ProfileScorer:
    ...
    def score(self, candidate: CandidateProfile, job: JobProfile) -> ScoringResult:
        ...
        return ScoringResult(breakdown=breakdown, evaluations=all_evaluations, gaps=gaps)
    ...
    def _compute_skill_semantic_score(self, requirement: JobRequirement, candidate_index) -> float | None:
        ...
        try:
            ...
            logger.warning(
                "scoring.semantic_unavailable dimension=skills requirement=%s", requirement.raw_text
            )
            return None
    ...
    def _compute_responsibility_semantic_score(
        ...
        try:
            ...
            logger.warning("scoring.semantic_unavailable dimension=responsibilities")
            return None, None
